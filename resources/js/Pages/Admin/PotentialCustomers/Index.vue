  ...
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <div class="flex-1 min-w-0 flex flex-col overflow-hidden w-full">
          ...
          <div class="flex items-center justify-between px-4 py-3">
            ...
            <div class="flex-1 text-center min-w-0">
              <h1 class="text-lg font-semibold text-gray-900 flex items-center justify-center gap-2 truncate">
                <span class="truncate">Potential Customers</span>
        ...
        <div class="hidden lg:block">
          <div class="px-4 lg:px-6 py-4">
            <div class="flex justify-between items-center">
              <div class="min-w-0">
                <h1 class="text-xl lg:text-2xl font-bold text-gray-900 flex items-center gap-2 lg:gap-3 truncate">
                  <span class="truncate">Potential Customers</span>
                  <div class="w-6 h-6 lg:w-7 lg:h-7 bg-gradient-to-r from-blue-400/30 to-purple-400/30 rounded-lg flex items-center justify-center flex-shrink-0">
                    <UserGroupIcon class="w-4 h-4 text-blue-600/70" />
                  </div>
                </h1>
                <p class="text-gray-600 mt-1 truncate text-sm lg:text-base">Manage and review potential customer leads</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex-1 overflow-hidden flex flex-col">
        ...
        <div class="bg-white border-b border-gray-200 px-3 md:px-4 lg:px-5 py-3 md:py-4 filters-section w-full overflow-x-hidden">
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <!-- Left side: Status Filters -->
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <span class="text-sm font-medium text-gray-700 hidden lg:block whitespace-nowrap">Filter:</span>
                <div class="flex flex-wrap gap-1 md:gap-2">
                  <button 
                    v-for="filter in statusFilters"
                    :key="filter.key"
                    @click="setStatusFilter(filter.key)"
                    class="inline-flex items-center px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 shadow-sm hover:shadow whitespace-nowrap"
                  ...
                  >
                    ...
                    <component 
                      ...
                      class="ml-1.5 w-3 h-3 flex-shrink-0" 
                    />
                  </button>
                </div>
              </div>
            ...
            <div class="flex items-center gap-2 md:gap-3 flex-shrink-0">
              ...
              <div class="relative">
                <button
                  ...
                  class="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-white bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg hover:from-purple-600 hover:to-pink-600 transition-all duration-200 shadow-sm hover:shadow whitespace-nowrap"
                >
                  <MapPinIcon class="w-4 h-4" />
                  <span class="truncate hidden sm:inline">Location Filter</span>
                  <span class="truncate sm:hidden">Location</span>
                  <ChevronDownIcon class="w-4 h-4 transition-transform duration-200 flex-shrink-0" :class="{'rotate-180': isFilterExpanded}" />
                </button>

                <!-- Expanded Location Filters Dropdown -->
                <div v-if="isFilterExpanded" class="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                  <div class="p-4">
              ...
              <div v-if="hasActiveFilters" class="flex items-center">
                <button
                  @click="clearAllFilters"
                  class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-gradient-to-r from-red-500 to-orange-500 rounded-lg hover:from-red-600 hover:to-orange-600 transition-all duration-200 shadow-sm hover:shadow whitespace-nowrap"
                >
                  <XMarkIcon class="w-3 h-3 mr-1" />
                  <span class="hidden sm:inline">Clear Filters ({{ activeFilterCount }})</span>
                  <span class="sm:hidden">Clear ({{ activeFilterCount }})</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div v-if="hasActiveFilters && activeFilterTags.length > 0" class="mt-2">
            <div class="flex flex-wrap gap-1.5">
              <div 
                ...
                :key="tag.key"
                class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium border shadow-sm whitespace-nowrap max-w-full"
              >
                <component :is="tag.icon" class="w-3 h-3 mr-1.5 flex-shrink-0" />
                <span class="truncate">{{ tag.label }}</span>
                <button @click="tag.remove()" class="ml-1.5 hover:opacity-75 flex-shrink-0">
        ...
        <div class="flex-1 overflow-auto">
          <div class="p-3 md:p-4 lg:p-5">
            ...
            <!-- Empty State -->
            <div v-else-if="!hasCustomers" class="bg-white rounded-xl md:rounded-lg border border-gray-200 p-4 md:p-6 lg:p-8 text-center shadow-sm max-w-full">
              <div class="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br from-blue-100 to-blue-200 rounded-xl flex items-center justify-center mx-auto mb-3 md:mb-4">
                <UserGroupIcon class="w-6 h-6 md:w-8 md:h-8 text-blue-600" />
              </div>
              <h3 class="text-base md:text-lg font-semibold text-gray-900 mb-1.5 md:mb-2">No potential customers found</h3>
              ...
              <button 
                @click="clearAllFilters"
                class="inline-flex items-center px-3 py-2 md:px-4 md:py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-xs md:text-sm font-medium rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-200 shadow-sm hover:shadow whitespace-nowrap"
              >
            ...
            <!-- Customers Content -->
            <div v-else class="space-y-3 md:space-y-4 max-w-full">
              <!-- Stats Summary (Mobile) -->
              <div class="md:hidden bg-white rounded-xl border border-gray-200 p-3 shadow-sm">
                <div class="flex items-center justify-between gap-2">
                  <div class="text-xs font-medium text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full whitespace-nowrap">
                    {{ filteredCustomers.length }} of {{ totalCustomers }} results
                  </div>
              ...
              <!-- Table Container -->
              <div class="bg-white rounded-xl md:rounded-lg border border-gray-200 overflow-hidden shadow-sm w-full">
                <!-- Table Summary -->
                <div class="px-3 md:px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                  <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                    <div class="min-w-0">
                      <h3 class="text-sm font-semibold text-gray-900 truncate">Customer Leads</h3>
                      <p class="text-gray-600 text-xs mt-0.5 truncate">
                        Showing {{ filteredCustomers.length }} of {{ totalCustomers }} results
                        <span v-if="statusFilter !== 'all'" class="ml-2 px-1.5 py-0.5 rounded text-xs whitespace-nowrap" :class="getFilterButtonClass(statusFilter)">
                ...
                <!-- Mobile Card View -->
                <div class="md:hidden">
                  <div v-for="customer in filteredCustomers" :key="customer?.id" class="border-b border-gray-200 p-3 hover:bg-blue-50/30 transition-all duration-150">
                    <!-- Card Header -->
                    <div class="flex items-start justify-between mb-2 gap-2">
                      ...
                      <div class="flex flex-col items-end space-y-0.5 flex-shrink-0">
                        <div class="text-xs text-gray-600 font-medium whitespace-nowrap">Updated</div>
                        <div class="text-xs text-gray-500 whitespace-nowrap">{{ formatDate(customer?.updated_at) }}</div>
                      </div>
                    </div>
                    ...
                    <div class="space-y-1.5 mb-2">
                      <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-700 truncate">{{ customer?.email || 'No email' }}</span>
                      </div>
                      <div class="flex items-center space-x-2">
                    ...
                    <div class="mb-2">
                      <div class="space-y-1">
                        ...
                        <!-- Text Location -->
                        <div v-if="customer?.text_location" class="flex items-center text-gray-500 text-xs">
                          <MapPinIcon class="w-3 h-3 mr-1.5 text-gray-400 flex-shrink-0" />
                          <span class="truncate" :title="customer.text_location">
                            {{ truncateText(customer.text_location, 25) }}
                          </span>
                        </div>
                        <!-- Map Location Link -->
                        <div v-if="customer?.map_location" class="flex items-center text-gray-500 text-xs">
                          <GlobeAltIcon class="w-3 h-3 mr-1.5 text-green-500 flex-shrink-0" />
                          <a 
                            :href="customer.map_location" 
                            target="_blank" 
                            rel="noopener noreferrer"
                            class="text-blue-600 hover:text-blue-800 truncate"
                            :title="customer.map_location"
                          >
                            View on Map
                          </a>
                        </div>
                        <div v-if="!customer?.text_location && !customer?.map_location" class="text-gray-400 text-xs italic">
                    ...
                    <div class="flex items-center justify-between">
                      <div class="text-xs text-gray-600 font-medium truncate max-w-[140px]">
                        Created by: {{ customer.created_by?.name || 'System' }}
                      </div>
                      <div class="flex items-center space-x-1.5 flex-shrink-0">
                        <button 
                          @click="viewCustomerDetails(customer)"
                          class="bg-blue-500 hover:bg-blue-600 text-white p-1 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow min-w-[28px] h-7"
                          title="View Details"
                        >
                        ...
                        <button 
                          ...
                          class="bg-green-500 hover:bg-green-600 text-white p-1 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow min-w-[28px] h-7"
                          title="Edit Customer"
                        >
                        ...
                        <button 
                          ...
                          class="bg-red-500 hover:bg-red-600 text-white p-1 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow min-w-[28px] h-7"
                ...
                <div class="hidden md:block overflow-x-auto">
                  <table class="w-full divide-y divide-gray-200 min-w-[1000px]">
                    <thead class="bg-gray-50">
                      <tr>
                        <th scope="col" class="px-3 py-2 lg:px-4 lg:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">
                      ...
                      >
                        ...
                        <td class="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap">
                          <div class="flex items-center">
                            ...
                            <div class="ml-3 min-w-0 max-w-[180px]">
                              <div class="font-semibold text-gray-900 text-sm truncate">
                                {{ customer?.potential_customer_name || 'Unknown Customer' }}
                              </div>
                              <div class="text-gray-500 text-xs truncate">
                        ...
                        <!-- Contact Column -->
                        <td class="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap max-w-[150px]">
                          <div class="space-y-1">
                            <div class="flex items-center text-gray-900 text-sm">
                              <EnvelopeIcon class="w-3 h-3 mr-1.5 text-gray-400 flex-shrink-0" />
                              <span class="truncate">{{ customer?.email || 'No email' }}</span>
                            </div>
                            <div class="flex items-center text-gray-500 text-xs">
                              <PhoneIcon class="w-3 h-3 mr-1.5 text-gray-400 flex-shrink-0" />
                              <span class="truncate">{{ customer?.phone || 'No phone' }}</span>
                            </div>
                          </div>
                        </td>

                        <!-- Location Column -->
                        <td class="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap max-w-[180px]">
                          <div class="space-y-1">
                            <!-- City and Subcity -->
                            <div class="flex items-center text-gray-900 text-sm">
                              <BuildingOfficeIcon class="w-3 h-3 mr-1.5 text-blue-500 flex-shrink-0" />
                        ...
                        <!-- Status Column -->
                        <td class="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap max-w-[120px]">
                        ...
                        <td class="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap">
                          <div class="flex gap-1">
                            <button 
                              @click="viewCustomerDetails(customer)"
                              class="bg-blue-500 hover:bg-blue-600 text-white p-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow min-w-[32px] h-8"
                              title="View Details"
                            >
                            ...
                            <button 
                              v-if="permissions.edit"
                              @click="openEditModal(customer)"
                              class="bg-green-500 hover:bg-green-600 text-white p-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow min-w-[32px] h-8"
                              title="Edit Customer"
                            >
                            ...
                            <button 
                              v-if="permissions.delete"
                              @click="deleteCustomer(customer.id)"
                              class="bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow min-w-[32px] h-8"
                              title="Delete Customer"
                            >
                ...
                <!-- Pagination -->
                <div v-if="potentialCustomers.links && potentialCustomers.links.length > 3" class="px-3 md:px-4 py-3 bg-gradient-to-r from-gray-50 to-white border-t border-gray-200">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <div class="flex items-center gap-1">
                        <button 
                          ...
                          class="min-w-[32px] h-8 px-2 text-xs font-medium rounded-lg transition-all duration-200 shadow-sm whitespace-nowrap"
                          :class="link.active 
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg p-4 md:p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-gray-200 shadow-xl mx-4">
        <!-- Modal Header -->
        <div class="flex justify-between items-center mb-4 md:mb-6 pb-4 border-b border-gray-200">
          <div class="min-w-0">
            <h2 class="text-xl md:text-2xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent truncate">
              Edit Potential Customer
            </h2>
            <p class="text-gray-600 text-xs md:text-sm mt-1 truncate">Update customer information and location details</p>
          </div>
          <button 
            @click="closeEditModal"
            class="text-gray-400 hover:text-gray-600 p-2 rounded-full hover:bg-gray-100 transition-colors flex-shrink-0"
          >
          ...
          <div class="bg-green-50 border-l-4 border-green-500 p-4">
            <div class="flex">
              ...
              <div class="ml-3 min-w-0">
                <p class="text-sm text-green-700 truncate">{{ successMessage }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="errorMessage" class="mb-4">
          <div class="bg-red-50 border-l-4 border-red-500 p-4">
            <div class="flex">
              ...
              <div class="ml-3 min-w-0">
                <p class="text-sm text-red-700 truncate">{{ errorMessage }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Form -->
        <form @submit.prevent="submitEdit" class="space-y-4 md:space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div class="md:col-span-2">
              ...
              <input
                ...
                class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 text-sm"
                required
              />
              <div v-if="editForm.errors.potential_customer_name" class="text-red-600 text-xs md:text-sm mt-1 truncate">
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 text-sm"
              />
              <div v-if="editForm.errors.email" class="text-red-600 text-xs md:text-sm mt-1 truncate">
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 text-sm"
              />
              <div v-if="editForm.errors.phone" class="text-red-600 text-xs md:text-sm mt-1 truncate">
            ...
            <!-- Text Location -->
            <div>
              <label for="text_location" class="block text-sm font-medium text-gray-700 mb-2">
              ...
              <input
                ...
                class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 text-sm"
                placeholder="e.g., Near Central Park, Downtown"
              />
              <div v-if="editForm.errors.text_location" class="text-red-600 text-xs md:text-sm mt-1 truncate">
                {{ editForm.errors.text_location }}
              </div>
            ...
            <!-- Map Location -->
            <div>
              <label for="map_location" class="block text-sm font-medium text-gray-700 mb-2">
                Map Location Link
              </label>
              <input
                type="url"
              ...
              />
              <div v-if="editForm.errors.map_location" class="text-red-600 text-xs md:text-sm mt-1 truncate">
                {{ editForm.errors.map_location }}
              </div>
              <p class="text-xs text-gray-500 mt-1 truncate">
            ...
            <div>
              ...
              <div v-if="editForm.errors.city_id" class="text-red-600 text-xs md:text-sm mt-1 truncate">
            ...
            <div>
              ...
              <select
                ...
                class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 text-sm disabled:bg-gray-100 disabled:cursor-not-allowed"
              >
              ...
              <div v-if="editForm.errors.subcity_id" class="text-red-600 text-xs md:text-sm mt-1 truncate">
            ...
            <div class="md:col-span-2">
              ...
              <textarea
                ...
                rows="3"
                class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 resize-none text-sm"
                placeholder="Add any additional remarks or notes about this customer..."
              ></textarea>
              <div v-if="editForm.errors.remarks" class="text-red-600 text-xs md:text-sm mt-1 truncate">
            ...
            <div class="md:col-span-2">
              ...
              <div v-if="editForm.errors.status" class="text-red-600 text-xs md:text-sm mt-1 truncate">
          ...
          <!-- Action Buttons -->
          <div class="flex flex-wrap gap-2 md:gap-3 mt-6 md:mt-8 pt-4 md:pt-6 border-t border-gray-200">
            <button
              type="submit"
              :disabled="editForm.processing"
              class="bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white px-4 md:px-6 py-2 rounded-lg font-semibold transition-all duration-200 flex items-center space-x-2 shadow hover:shadow-lg text-sm whitespace-nowrap"
            >
            ...
            <button
              type="button"
              @click="closeEditModal"
              class="bg-gray-600 hover:bg-gray-700 text-white px-4 md:px-6 py-2 rounded-lg font-semibold transition-all duration-200 flex items-center space-x-2 shadow hover:shadow-lg text-sm whitespace-nowrap"
            >
              <XMarkIcon class="w-4 h-4" />
              <span>Cancel</span>
            </button>
            ...
            <button
              type="button"
              @click="resetEditForm"
              class="bg-amber-600 hover:bg-amber-700 text-white px-4 md:px-6 py-2 rounded-lg font-semibold transition-all duration-200 flex items-center space-x-2 shadow hover:shadow-lg text-sm whitespace-nowrap"
            >
              <ArrowPathIcon class="w-4 h-4" />
              <span>Reset</span>
            </button>
...
const openEditModal = (customer) => {
  ...
  editForm.text_location = customer.text_location || ''
  editForm.map_location = customer.map_location || ''
...
const resetEditForm = () => {
  if (selectedCustomer.value) {
    ...
    editForm.text_location = selectedCustomer.value.text_location || ''
    editForm.map_location = selectedCustomer.value.map_location || ''
